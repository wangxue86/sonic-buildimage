...
fan_adj_list = [
    ...
    {'type': 'max6696' ,'index': 0, 'spot': 1 , 'Th': 67, 'Tl': 45, 'Nl': 0x27, 'Nh': 0xc2, 'k': 7.05,  'Twarn': 75, 'TLwarn':0, 'Tshut': None} ,
    {'type': 'max6696' ,'index': 0, 'spot': 2 , 'Th': 85, 'Tl': 55, 'Nl': 0x27, 'Nh': 0xc2, 'k': 5.17,  'Twarn': 88, 'TLwarn':0, 'Tshut': 107},
    ...
    #previous is not hardware reboot
        ...
        for match in all_reason:
            if "RESET_TYPE_COLD" in match:
                reboot_reason_string += "cold boot/power down\n"
            if "RESET_TYPE_WDT" in match:
                reboot_reason_string += "CPLD watchdog reboot\n"
            if "RESET_TYPE_BOOT_SW" in match:
                reboot_reason_string += "BIOS switchover reboot\n"
    ...
    def manage_temperture_monitor(self):
        ...
        for spot_info in fan_adj_list:
            ...
            if temp_shut != None and temp_input >= temp_shut:
                ...
                self.record_reboot_cause("Over temperature reboot at %s" %(str(datetime.datetime.now())))
                if (self.over_temp_reboot == 1):
            ...
            if status == STATUS_NORMAL:
                ...
                try:
                     ...
                     if voltage_in <= PSU_AC_IN_VOL_LOW:
                        log_notice('%%PMON-5-VOLTAGE_HIGH: psu%d voltage %.2fV is lower than min threshold %sV' %(i + 1, voltage_in, PSU_AC_IN_VOL_LOW))
                     elif voltage_in >= PSU_AC_IN_VOL_HI:
                         log_notice('%%PMON-5-VOLTAGE_HIGH: psu%d voltage %sV is larger than max threshold %sV' %(i + 1, voltage_in, PSU_AC_IN_VOL_HI))

...
extern int optic_lock(int optic_index);
extern void optic_unlock(void);
...
struct optoe_data {
	...
	/*	
	 *	wangdongwen
	 *	add port index and h3client[2]
	 */
	int port_index;
	struct i2c_client h3client[2];
	/*
 ...
 * Get Optic index
 */
static int optoe_get_client_index(struct optoe_data *optoe,
		    struct i2c_client *client)
{
	if(1 == optoe->num_addresses)
	{
		return 0;
	}
	else
	{
		if(client == optoe->client[0])
			return 0;
		else
			return 1;
	}
		    ...
		    char *buf, unsigned int offset, size_t count)
{
	...
	/* wangdongwen:
	 * Select Optic and process mutex lock
	 */
	if( optic_lock(optoe->port_index) )
	{
		optic_unlock();
		return -EIO;
	}
	client_index = optoe_get_client_index(optoe, client);
	client = optoe->h3client + client_index;
	/*
	printk("	optoe->port_index %d		client_index %d read optoe->h3client %p	client %p 	client->addr=%#x	optoe->h3client.addr=%#x\n", \
				optoe->port_index,			client_index,		 optoe->h3client,	client, client->addr,		optoe->h3client[0].addr);
    */
	...
	do {
		...
		case I2C_SMBUS_BYTE_DATA:
		/*
			printk("optoe_eeprom_read I2C_SMBUS_BYTE_DATA for client %p and client->addr=%#x\n", client, client->addr);
		*/
			...
			const struct i2c_device_id *id)
{
	...
	/*printk("wangdongwen : client %#x probe\n", client->addr);*/
...
#else
	/* wangdongwen : 	port number validation (depend on board type, S6850 S9850 S9820 etc.)
	 *					and tell front pannel port number from client->addr.
	 *  				No client->addr value valiation here because i2c adapter code ensures client->addr >=1
	 */
		port_num = optoe_port_index_convert( (int)(client->addr) ); 
		if( -1 == port_num )
		{
			err = -ENODEV;
			goto exit;
...
/*
 *	wangdongwen :	this kzalloc is amazing, there is "struct i2c_client *client[]" struct pointer \
 *					defined at the end of struct optoe_data.
*/
	...
	/*
	 * wangdongwen : store client[0] struct in h3client[0]
	 */
	optoe->h3client[0] = *client;
	optoe->h3client[0].addr = 0x50;	
	/* wangdongwen:
	 * Record port_index[0] in optoe_data
	 */
	optoe->port_index = port_num-1;
	...
	if (num_addresses == 2) {
		...
		#else
		/*
		 * wangdongwen :	change client[1] addr and the sysfs node will be changed too. 				\
		 					For example, if client->addr=0x01, then /sys/class/i2c-adapter/i2c-0/0-0002 \
		 					will be created after calling i2c_new_dummy.
		 *
		 */
		optoe->client[1] = i2c_new_dummy(client->adapter, client->addr+1);
		#endif
		if (!optoe->client[1]) {
			#if 0
			dev_err(&client->dev, "address 0x51 unavailable\n");
			#else
			dev_err(&client->dev, "address %#x unavailable\n", client->addr+1);
			#endif
			err = -EADDRINUSE;
			goto err_struct;
		}
		/*
		 * wangdongwen : store client[1] struct in h3client[1]
		 */
		optoe->h3client[1] = *(optoe->client[1]);
		optoe->h3client[1].addr = 0x51;
	}
	else
	{

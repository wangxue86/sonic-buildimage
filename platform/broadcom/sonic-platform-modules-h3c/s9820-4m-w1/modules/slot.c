...
int slot_debug_level = DEBUG_INFO | DEBUG_ERR;

#define DBG_ECHO(level, fmt, args...) DEBUG_PRINT(slot_debug_level, level, fmt,##args)
...
static ssize_t bsp_slot_sysfs_get_attr(struct device *kobjs, struct device_attribute *da, char *buf)
{
    ...
    switch (attr->index)
    {
        ...
        case PN:
        case PRODUCT_NAME:
            if (ERROR_SUCCESS == bsp_slot_get_eeprom(slot_index, eeprom_buff, sizeof(eeprom_buff)))
            {
            ...
            {
                DBG_ECHO(DEBUG_ERR, "bsp_slot_get_eeprom failed!");
            }
            break;
        case HW_VERSION:
        ...
        case NUM_TEMP_SENSORS:
            ...
            if (subcard != NULL && subcard->initialized)
            {
...
static ssize_t bsp_slot_sysfs_set_attr(struct device *kobjs, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    switch (attr->index)
    {
        case LED_STATUS:
            ...
            else if (ERROR_SUCCESS != bsp_set_led_color_for_slot(temp, slot_index))
            {
...
void release_all_slot_kobj()
{
    ...
    if (kobj_slot_root != NULL && kobj_slot_root->state_initialized)

...
# chassis_common.py
#
# Common chassis class for common implementations
#
#
...
class ChassisCommon(ChassisBase):
    """
    Common chassis class, must be inited with args:
        component_list,
        fan_list,
        psu_list,
        thermal_list,
    ...
    def __init__(self):
        super(ChassisCommon, self).__init__()
    ...
    def get_name(self):
        ...
        if name is None:
            self.logger.log_error('syseeprom product name is error.')
            return ''
        return name

    def get_platform_name(self):
        """
        Retrieves the platform name for the chassis

        Returns:
            A string containing the platform name in the format
        """
        name = ''
        sys_eeprom = self.get_eeprom()
        if sys_eeprom is None:
        ...
        e = sys_eeprom.read_eeprom()
        platform_name = sys_eeprom.platform_name(e)
        if platform_name is None:
            self.logger.log_error('syseeprom platform name is error.')
            return ''
    ...
    def get_model(self):
        ...
        e = sys_eeprom.read_eeprom()
        model = sys_eeprom.part_number_str(e)
        if model is None:
    ...
    def get_switch_mac(self):
        ...
        if switch_mac is None:
            self.logger.log_error('syseeprom switch mac is error.')
            return ''
    ...
    def get_change_event(self, timeout=0, scantime=0.5, dev_list=['sfp']):
        ...
        while True:
            ...
            for dev in dev_list:
                ...
                self.logger.log_debug('get {} event'.format(dev))
                if dev == 'voltage':
    ...
    def get_sfp(self, index):
        ...
        Note: the index from 1, because chassiscommon port_config.ini form Ethernet1
        """
        sfp = None
        index -= 1
        ...
        try:
            ...
            self.logger.log_error("SFP index {} out of range (0-{})\n".format(

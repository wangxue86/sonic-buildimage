...
class SfpUtil(SfpUtilBase):
    ...
    BASE_OPTOE_PATH = "/sys/class/i2c-adapter/i2c-0/0-{:0>4x}/"
    _port_to_is_present = {}
    _port_to_lp_mode = {}
    ...
    def __init__(self):
        ...
        for x in range(self.port_start, self.port_end+1):
            ...
            slot  = (x - 1) / self.PORT_NUM_PER_SLOT + 1
            port = (x - 1) % self.PORT_NUM_PER_SLOT + 1
            
            self._port_base_path[x] = self.BASE_VAL_PATH.format(speed, slot, port)
            self._port_optoe_base_path[x] = self.BASE_OPTOE_PATH.format(x)
